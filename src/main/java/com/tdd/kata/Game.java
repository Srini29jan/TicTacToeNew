...
public class Game {
    ...
    public char getWinner() {
        if (isAnyRowFilledByPlayer(PLAYER_X) || isFirstColumnFilledByPlayer(PLAYER_X)) {
            return PLAYER_X;
        }

        if (isAnyRowFilledByPlayer(PLAYER_O) || isFirstColumnFilledByPlayer(PLAYER_O)) {
            return PLAYER_O;
        }

        if (isSecondColumnFilledByPlayerO(PLAYER_X)) {
            return PLAYER_X;
        }

        if (isSecondColumnFilledByPlayerO(PLAYER_O)) {
            return PLAYER_O;
        }
